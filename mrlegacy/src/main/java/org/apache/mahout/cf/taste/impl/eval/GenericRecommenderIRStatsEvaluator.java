...
public IRStatistics evaluate(RecommenderBuilder recommenderBuilder,
    ...
    Preconditions.checkArgument(dataModel != null, "dataModel is null");
    Preconditions.checkArgument(at >= 1, "at must be at least 1");
    Preconditions.checkArgument(evaluationUsers.size() > 0,
        "Invalid evaluation users: " + evaluationUsers.size() + ". Number of users must be greater than 0.");
    ...
    LongPrimitiveIterator it = evaluationUsers.iterator();
    while (it.hasNext()) {
